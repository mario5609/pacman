...
export default function Pinky() {
  const { pinky, setPinky, level, setlevel, lost, setLost, status, setStatus } = useContext(Context)
  ...
  let PinkyDirection = Math.floor(Math.random() * 4)

  function movePinky() {
    let pink = { ...pinky };
    let newLevel = [...level];

    if ((newLevel[pink.y][pink.x] !== 3) && (newLevel[pink.y][pink.x] !== 4)) {
      newLevel[pink.y][pink.x] = 2;
    }

    if ((newLevel[pink.y][pink.x - 1] == 5) || (newLevel[pink.y - 1][pink.x] == 5) || (newLevel[pink.y][pink.x + 1] == 5) || (newLevel[pink.y + 1][pink.x] == 5)) {
      setLost(true)
    }
    switch (PinkyDirection) {
      case 0: //left
        if ((newLevel[pink.y][pink.x - 1]) != 1 && (newLevel[pink.y][pink.x - 1]) != 7 && (newLevel[pink.y][pink.x - 1]) != 6 && (newLevel[pink.y][pink.x - 1]) != 8) {
          pink.x--;
        }
        break;
      case 1: //up
        if ((newLevel[pink.y - 1][pink.x]) != 1 && (newLevel[pink.y - 1][pink.x]) != 7 && (newLevel[pink.y - 1][pink.x]) != 6 && (newLevel[pink.y - 1][pink.x]) != 8) {
          pink.y--;
        }
        break;
      case 2: //right
...
/* pinky color
background: linear-gradient(rgb(254, 45, 251), rgb(250, 206, 243));}
fill="rgb(250, 190, 243)"
*/