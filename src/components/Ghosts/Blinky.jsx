...
export default function Blinky() {
    const { blinky, setBlinky, level, setlevel, lost, setLost, status, setStatus } = useContext(Context)
    ...
    let blinkyDirection = Math.floor(Math.random() * 4)
    ...
    function moveBlinky() {
        ...
        if ((newLevel[red.y][red.x] !== 3) && (newLevel[red.y][red.x] !== 4)) {
            newLevel[red.y][red.x] = 2
        }

        if ((newLevel[red.y][red.x - 1] == 5) || (newLevel[red.y - 1][red.x] == 5) || (newLevel[red.y][red.x + 1] == 5) || (newLevel[red.y + 1][red.x] == 5)) {
            setLost(true)
        }

        switch (blinkyDirection) {
            case 0: //left
                if ((newLevel[red.y][red.x - 1]) != 1 && (newLevel[red.y][red.x - 1]) != 7 && (newLevel[red.y][red.x - 1]) != 8 && (newLevel[red.y][red.x - 1]) != 9) {
                    red.x--;
                }
                break;
            case 1: //up
                if ((newLevel[red.y - 1][red.x]) != 1 && (newLevel[red.y - 1][red.x]) != 7 && (newLevel[red.y - 1][red.x]) != 8 && (newLevel[red.y - 1][red.x]) != 9) {
                    red.y--;
                }
                break;
            case 2: //right
...
/* blinky color
background: linear-gradient(red, rgb(252, 149, 111));
fill="rgb(252, 135, 120)"
*/
